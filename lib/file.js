...
module.exports.setup = function (levelMap, path) {
	...
	// set up listener on the log directory
	fs.watch(path, function (event, filename) {
		if (event === 'rename') {
			// the log file name has changed or removed
			var levelName = filename.substring(0, filename.indexOf('.'));
			destroyWriteStream(levelName);
			getWriteStream(levelName);
		}
	});
