...
module.exports.create = function (prefix, logName, levelName, args) {
    if (stackdriver) {
      return { message: args.join(''), timestamp: new Date() };
    }
    var timestamp;
    var date = new Date();
    if (useTimestamp) {
    ...
    if (typeof msgItem === 'object') {
        if (msgItem instanceof Error) {
            if (!compress) {
                ...
                msgItem = msgItem.message + '\t<stack trace>\t' + msgItem.stack.replace(/\n/g, '\t');
            }
        } else {
            if (compress) {
                msgItem = util.format('%j', msgItem);
            } else {
